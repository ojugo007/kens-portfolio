...
const Home = () => {
  ...
  return (
    <div className="bg-base h-full overflow-auto scrollbar-hide scroll-smooth">
      <main
        className="flex min-h-screen bg-fixed bg-cover bg-center relative scroll-smooth"
        style={{
          backgroundImage: `linear-gradient(var(--overlay-start), var(--overlay-end)), url('https://res.cloudinary.com/dw94gbpfs/image/upload/v1780168312/lin2015-siguniang-mountain-8568913_1920_jsgqgu.jpg')`,
      ...
      >
        ...
        <Card
          className={`
            w-64.5 rounded-none text-ink border-0 shadow-none ring-0 pt-0
          ...
          `}
        >
          <CardHeader className="border-0 pt-6 pb-12 bg-surface rounded-none shrink-0 relative">
                ...
                {Object.entries(stack).map(([key, value]) => (
                  <div key={key}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <small className="text-[12px] text-ink-muted">{value}%</small>
                    </div>
                    <Progress value={stackProgress[key]} className="max-w-full bg-card-alt flex-1 rounded-none" />
                  </div>
                ))}
              </div>
              <Separator className="bg-edge my-4" />
              ...
              <div className="space-y-2">
                {skills.map((group, index) => (
                  <div key={index} className="flex items-center justify-start gap-2">
                    <Check size={11} strokeWidth={5} className="shrink-0 text-accent" />
                    <p className="text-[12px] text-ink-muted font-light text-left">{group.items.join(", ")}</p>
                  </div>
                ))}
              </div>
              <Separator className="bg-edge my-4" />
        ...
        <div className="w-full flex-1 z-10 p-4 lg:p-6.5 text-ink min-h-screen overflow-x-hidden">
          ...
          {/* Mobile top bar */}
          <div className="lg:hidden flex items-center justify-between mb-4 bg-surface px-4 py-3 -mx-4 -mt-4">
            <button
          ...
          {/* Hero — text stays white; the photo overlay behind it stays dark in both themes */}
          <header
            ...
            <div className="relative z-10 p-4 lg:p-6 text-white flex w-full gap-4">
              <div className="text-left w-full lg:w-[60%]">
                ...
                <div className="mt-7 sm:mt-3 flex gap-3">
                  ...
                  <a href="#projects"
                    className="px-4 py-2 bg-accent w-32 lg:w-37.5 text-[#382400] uppercase text-[11px] font-bold tracking-widest cursor-pointer transition-all duration-600 ease-in-out text-center hover:tracking-wider"
                  >
                    explore now
                  </a>
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {projects.map((project, index) => (
                <div onClick={() => setSelected(project.id)} className="overflow-hidden relative transition-all group cursor-pointer h-50" key={index}>
                  ...
                  />
                  <div
                    className={`absolute z-10 right-0 bottom-0 w-full bg-card/98 p-4 text-left
          ...
          <section id="resume" className="my-6 w-full">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1">
                <h3 className="text-left text-[16px] font-semibold mb-6 text-ink/80">Education</h3>
                <div className="flex gap-3">
                  <div className="flex-1 flex flex-col gap-4">
                    {EDUCATION.map((e, i) => (
                      <div key={i} className="bg-card p-5 flex flex-col gap-2">
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <p className="text-[13px] text-ink/80 font-semibold leading-5">{e.school}</p>
                          <span className="shrink-0 text-[10px] text-ink-muted bg-card-alt px-2.5 py-1 rounded-full">{e.period}</span>
                        </div>
                        <p className="text-[12px] text-ink-muted italic text-left">{e.role}</p>
                        <p className="text-[12px] text-ink-muted leading-4.5 text-left">{e.desc}</p>
                        {e.cta && (
                          <a href={e.ctaUrl ?? "#"} className="mt-1 text-accent text-[10px] tracking-widest uppercase flex items-center gap-1 hover:gap-2 transition-all duration-300">
                  ...
                  <div className="flex flex-col items-center pt-1">
                    {EDUCATION.map((_, i) => (
                      <React.Fragment key={i}>
                        <div className="w-3 h-3 rounded-full border-2 border-accent bg-transparent shrink-0" />
                        {i < EDUCATION.length - 1 && <div className="w-px flex-1 min-h-25 bg-edge" />}
                      </React.Fragment>
                    ))}
                  </div>
                </div>
              ...
              {/* Experience */}
              <div className="flex-1">
                <h3 className="text-left text-[15px] font-semibold mb-6 text-ink/80">Work History</h3>
                <div className="flex gap-3">
                  <div className="flex flex-col items-center pt-1">
                    {EXPERIENCE.map((_, i) => (
                      <React.Fragment key={i}>
                        <div className="w-3 h-3 rounded-full border-2 border-accent bg-transparent shrink-0" />
                        {i < EXPERIENCE.length - 1 && <div className="w-px flex-1 min-h-25 bg-edge" />}
                      </React.Fragment>
                    ))}
                  </div>
                  <div className="flex-1 flex flex-col gap-4">
                    {EXPERIENCE.map((e, i) => (
                      <div key={i} className="bg-card p-5 flex flex-col gap-2">
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <p className="text-[13px] text-ink/80 font-semibold leading-5">{e.company}</p>
                          <span className="shrink-0 text-[10px] text-ink-muted bg-card-alt px-2.5 py-1 rounded-full">{e.period}</span>
                        </div>
                        <p className="text-[12px] text-ink-muted italic text-left">{e.role}</p>
                        <p className="text-[12px] text-ink-muted leading-4.5 text-left">{e.desc}</p>
